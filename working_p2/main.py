...
LEARNING_RATE = 5e-4
LOG_DIR = './logs/x_log' + str(LEARNING_RATE)
LOG_INTERVAL = 1000
SAVE_PATH = './x_model.pack'.format(LEARNING_RATE)
SAVE_INTERVAL = 10000
    ...
    def q_evolution(self):
        ...
        if a==b:
            if self.type == '=':
                ...
                if self.pd[0] <= 0.5:
                    self.pd[0] += np.random.random(1)/5
                if self.pd[1] >= 0.1:
                    self.pd[1] -= np.random.random(1)/10
                self.pd[2] = 1- self.pd[0] - self.pd[1]

            elif self.type == '<':
                if self.pd[2] <= 0.5:
                    self.pd[2] += np.random.random(1)/5
                if self.pd[0] >= 0.1:
                    self.pd[0] -= np.random.random(1)/10
...
if __name__ == '__main__':
    ...
    summary_writer = SummaryWriter(LOG_DIR+env.type)
    ...
    for step in itertools.count():
        ...
        if step % LOG_INTERVAL == 0:
            ...
            summary_writer.add_scalars('Done Flags', {'flag_0':env.done_flags[0],'flag_1':env.done_flags[1]})
